...
# 대표 기사 선정 (가장 중심에 가까운 기사)
    ...
    related_articles = []  # 연관 뉴스 (rep_oid_aid, related_oid_aid, similarity)
# ------------------------------------------------------------
    # 로그 및 유사도 쌍 기록    
    ...
    for i in range(len(indices)):
            ...
            if sim >= THRESHOLD_CONTENT:
                ...
                # oid_aid 추출하여 연관 뉴스 저장
                rep_oid_aid = df.loc[rep_idx, 'oid_aid'] if 'oid_aid' in df.columns else f"idx_{rep_idx}"
                related_oid_aid = df.loc[idx, 'oid_aid'] if 'oid_aid' in df.columns else f"idx_{idx}"
                related_articles.append((rep_oid_aid, related_oid_aid, round(float(sim), 4)))  # 연관 뉴스
            else:
                pass
